...
function AdminEnterNewPassword() {
  const { id } = useParams();
  ...
  const confirmForgetPasswordToken = async () => {
    try {
      setLoading(true);
      const response = await authService.adminConfirmForgetPasswordToken(id);
  ...
  const makeRequest = async () => {
    try {
      setLoading(true);
      const response = await authService.adminEnterNewPassword(
        inputFields.email,
